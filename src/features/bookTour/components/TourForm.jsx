...
export let locations = [
  ...
  {
    postalCode: "32176",
    clubName: "South Edmonton Common",
    clubAddress: "# 1910 102 St NW Edmonton, Alberta, T6N 1N3",
    accountId: "20534",
  },
  {
    postalCode: "40142",
...
export default function TourForm() {
  ...
  useEffect(() => {
    // Read location from query param (?location=40142) without reload
    const readQueryLocation = () => {
      if (typeof window === "undefined") return;
      try {
        const url = new URL(window.location.href);
        setUrlLocation(url.searchParams.get("location"));
      } catch (_) {
        // Fallback for older environments
        const params = new URLSearchParams(window.location.search);
        setUrlLocation(params.get("location"));
      }
    };

    readQueryLocation();
    // Keep in sync when user navigates back/forward
    window.addEventListener("popstate", readQueryLocation);
    return () => window.removeEventListener("popstate", readQueryLocation);
  }, []);

  // Prefer locally tracked query param; fallback to Next's initial searchParams
  const location = urlLocation ?? searchParams.get("location");

  useEffect(() => {
    if (
      location &&
      location == "06962" &&
      locations.findIndex((loc) => loc.postalCode === "06962") === -1
    ) {
      locations.push({
        postalCode: "06962",
        clubName: "Edmonton South",
...
function FormCard({ onSuccess, location = false }) {
  ...
  const SOUTH_EDMONTON_OPEN = new Date(2026, 4, 15); // May 15, 2026 — opening date
  ...
  const minTourDate =
    formData.gymPostalCode === "32176" && today < SOUTH_EDMONTON_OPEN
      ? SOUTH_EDMONTON_OPEN
      : today;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_APP_API_URL}/gymsales/tour`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          personPayload,
